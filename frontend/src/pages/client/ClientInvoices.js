...
const ClientInvoices = () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto" data-testid="client-invoices">
      <div className="mb-6">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-4 py-3 text-left text-sm font-semibold text-slate-600">Montant HT</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-slate-600">TVA</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-slate-600">Total TTC</th>
              ...
              {filtered.map((invoice, i) => {
                ...
                return (
                  <tr key={invoice.id} className={`border-b border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                    ...
                    <td className="px-4 py-3 text-sm text-slate-800">{invoice.subtotal.toFixed(2)}€</td>
                    <td className="px-4 py-3 text-sm text-slate-600">{invoice.tax_amount.toFixed(2)}€</td>
                    <td className="px-4 py-3">
